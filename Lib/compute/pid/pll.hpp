...
#define Motor_RPM (73) //电机最大转速 实测电机最高1.21圈/s =73rpm

#define Motor_PolesNub (21) //极对数
#define Motor_Fe (Motor_RPM*Motor_PolesNub/60) //
#define Wn_min  (M_2PI*Motor_Fe/10) //最小自然频率 = 2pi*Fe/10 = 15.71hz
...
#define Ts_max  (3.0/(PLL_ZETA*Wn_min)*1000) //最大收敛时间 Ts = 3/(ζ*Wn_min) = 270.13ms

#define PLL_FREQ_Dt   (1.0/FOC_VELOCITY_UP_FREQ_HZ)//采样周期
#define PLL_ts_temp   (40) //收敛时间 单位ms 
...
#define FOC_PLL_KP    (2*PLL_ZETA*PLL_Wn) //PI参数计算公式 kp = 2ζωn = 300
#define FOC_PLL_KI    (PLL_Wn*PLL_Wn) //PI参数计算公式 ki = ωn² = 45013.6
